...
- (void)viewDidLoad {
    ...
    if (_gnexus) {
        ...
        [self nextUrl:@"ftp://10.1.0.173/f_feeds/"];
        [self getPath:fmyURL];
    }
...
- (nonnull __kindof UICollectionViewCell *)collectionView:(nonnull UICollectionView *)collectionView cellForItemAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    if ([[array objectAtIndex:indexPath.row] hasPrefix:@"-"]) {
        ...
        NSString *ftpLocation = [NSString stringWithFormat:@"ftp://10.1.0.173/f_feeds/%@_%d/%d.jpg", nameGet, get3, get3];
...
-(void)nextUrl :(NSString *)str {
    if ([str isEqual: @"ftp://10.1.0.173/f_feeds/"]) {
    ...
    if ([_gurl isEqual:@"ftp://10.1.0.173/f_feeds/"]) {
...
-(void)nextFile :(NSString *)str {
    ...
    if ([fmyURL isEqual:@"ftp://10.1.0.173/f_feeds/"]) {
        [self.gbtn setEnabled:NO];
        self.gbtn.alpha = 0;
    } else {
        [self.gbtn setEnabled:YES];
...
-(IBAction)fback:(id)sender{
    if([_flblon.text isEqual:@"ftp://10.1.0.173/f_feeds/"]){
        _gbtn.alpha = 0;
        _gbtn2.alpha = 0;
